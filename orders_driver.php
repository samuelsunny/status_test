...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    if($_POST['delivered'])
    {
        // echo $_POST['delivered'];
        $trucking_order_id = $_POST['delivered'];
        $accepted = "Yes";
        ...
        if($result)
        {
            if($result && mysqli_num_rows($result) > 0)
            {
                $trucking_order_driverId = mysqli_fetch_all($result);
            }
        }
    ...
    {
        // echo $_POST['pickedup'];
        $trucking_order_id = $_POST['pickedup'];
        $accepted = "Yes";
        $query = "update truck_orders set pickedUp ='{$accepted}' where truckOrderId = '{$trucking_order_id}'";
    ...
    {
        // echo $_POST['returned'];
        $trucking_order_id = $_POST['returned'];
        $returned = "Yes";
        $query = "update truck_orders set ifLoadOnReturn ='{$returned}' where truckOrderId = '{$trucking_order_id}'";
                                ...
                                <div class="dropdown">
                                    <select class="form-select" aria-label="Default select example" onchange="location = this.value;" id="containerId">
                                        ...
                                        <?php for ($row = 0; $row < count($drivers_data); $row++) { ?>
                                            ...
                                            <option name="<?php echo $drivers_data[$row][0];?>" value="orders_driver.php?did=<?php echo $drivers_data[$row][0]; ?>" >
      ...
      <div class="row mt-4">
          ...
          <table class="table">
            <thead>
                ...
                <th scope="col" class="text-center">Delivery Warehouse Id</th>
                <th scope="col" class="text-center">Delivery pickup time</th>
            ...
            <?php for ($row = 0; $row < count($trucking_orders_data); $row++) {?>
              <tr>
                ...
                <th scope="row" class="text-center"><?php echo $trucking_orders_data[$row][8] ?></th>
                <th scope="row" class="text-center"><?php echo $trucking_orders_data[$row][20] ?></th>

                <td class="text-center">
                    <?php 
                        if($trucking_orders_data[$row][16] == "Yes" && $trucking_orders_data[$row][19] == "Yes" && $trucking_orders_data[$row][14] == 0 && $trucking_orders_data[$row][21] == 0 )
                        { ?>
                            <div class="row">
                                <div class="col-12">
                                    <button type="button" class="btn btn-success" disabled>
                                        <?php echo "Pickup done!"; ?>
                                    </button>
                                </div>
                            </div>
                            <td>
                                <form method="post" action="orders_driver.php">
                                    <input type="hidden" name="returned" id="poster" value="<?php echo $trucking_orders_data[$row][0]; ?>"/>  
                                    <button type="submit" class="btn btn-danger"> Returned </button> 
                                </form> 
                            </td>
                            <td>
                                <form method="post" action="orders_driver.php">
                                    <input type="hidden" name="delivered" id="poster" value="<?php echo $trucking_orders_data[$row][0]; ?>"/>  
                    ...
                    <?php 
                        if($trucking_orders_data[$row][16] == "Yes" && $trucking_orders_data[$row][19] == "Yes" && $trucking_orders_data[$row][21] == "Yes")
                        { ?>
                            <div class="row">
                                <div class="col-12">
                                    <button type="button" class="btn btn-success" disabled>
                    ...
                    <?php } ?>
                    <?php 
                        if($trucking_orders_data[$row][16] == "Yes" && $trucking_orders_data[$row][14] != 0 && $trucking_orders_data[$row][19] == "Yes")
                        { ?>
                            <div class="row">
                                <div class="col-12">
                    ...
                    <?php } ?>
                    <?php 
                        if($trucking_orders_data[$row][16] == "Yes" && $trucking_orders_data[$row][19] == 0)
                        { ?>
                            <div class="row">
                                <div class="col-12">
                                <form method="post" action="orders_driver.php">
                                    <input type="hidden" name="pickedup" id="pickup" value="<?php echo $trucking_orders_data[$row][0]; ?>"/>  
                                    <button type="submit" class="btn btn-warning"> Pickup </button> 
                                </form>
                                </div>
                            </div>
