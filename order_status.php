    ...
    if (sizeof($_GET,1) == 1)
    {
        ...
        if($result)
        {
            if($result && mysqli_num_rows($result) > 0)
            {
                ...
                $query = "select * from loading_orders where containerId = '{$order_container}'";
                $result = mysqli_query($con, $query);
                if($result && mysqli_num_rows($result) > 0)
                {
                ...
                $query = "select shipId,onShipDate,arrivalDate from shipping_order where containerId = '{$order_container}'";
                $result = mysqli_query($con, $query);
                if($result && mysqli_num_rows($result) > 0)
                {
                ...
                $query = "select ifLoadOnReturn from truck_orders where containerId = '{$order_container}'";
                $result = mysqli_query($con, $query);
                if($result && mysqli_num_rows($result) > 0)
                {
